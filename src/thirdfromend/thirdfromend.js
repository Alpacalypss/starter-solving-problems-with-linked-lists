 ...
 */
/*
Initialize two points, one to increment per iteration and the other to increment starting at third iteration
if first increment reaches the end of the list and is less than 3 return null
...
/* function thirdFromEnd(list) {
  let length = 0;
  let lengthBy3 = length - 3;
  let node = list.head;
  let thirdFromEnd = list.head;
  while (node) {
    length++;
    node = node.next;
  }
  if (lengthBy3 < 0) return null;
  return thirdFromEnd;
} */
